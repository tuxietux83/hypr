...
sudo apt install -y "${drivers[@]}"
sudo apt install -y "${fonts[@]}"
sudo apt install -y "${network[@]}"
sudo apt install -y "${audio[@]}"
sudo apt install -y "${appeareance[@]}"
...
read -p "Trying to install wayland-protocols, press enter to continue ..."
### Wayland-protocols latest ok
### subprject wayland not working ....
...
read -p "Trying to install wayland, press enter to continue ..."
### Wayland ok
...
echo "session.target and autostart.sh, theme.sh"
...
### !!! we dont care, we backup everything ... !!!
[ ! -d "$conf_dir"/backup ] && mkdir -pv "$conf_dir/backup"
[ -d "$conf_dir"/backup ] && mkdir -pv "$conf_dir/backup/$cmd_date"
## what we backup and restore:
conf_apps=(
	dunst
	hypr
	kitty
...
for configs in "${conf_apps[@]}"; do
	if [ -d "$conf_dir"/"$configs" ]; then
		mv -v "$conf_dir/$configs" "$conf_dir/backup/$cmd_date"
		cp -rv "dot/config/$configs" "$conf_dir/$configs"
	else
		cp -rv "dot/config/$configs" "$conf_dir/$configs"
	fi
	echo "config: $configs -> ok"
done

theme="Breeze-Dark"
echo "We set a dark theme: $theme, just because we can ..."
